...
**Multiple agents, one folder.** `cc-attach --name <task-name>` (lowercase letters/digits/`_`/`-`, e.g. `wb-sync`) gives that terminal its own chat/inbox/Overlord identity instead of sharing the folder's default one — so two or more `claude` sessions working different tasks in the same project directory can each be messaged individually. `CC_HUB_NAME=<task-name>` works the same way as an env var if you'd rather not pass a flag. Without `--name`/`CC_HUB_NAME`, behavior is unchanged: all unnamed terminals in one folder still share a single identity.
