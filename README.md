...
On Windows it drives `claude` through the **winpty** backend by default (ConPTY, relayed through a second terminal host, desyncs the cursor after the first keystroke — typed lines land in the wrong spot); set `CC_HUB_USE_CONPTY=1` to force ConPTY back if you prefer its rendering.
