...
- **Protocol** (localhost WS, `/attach` route in `app.ts`): wrapper→hub `{t:'register',cwd,pid}` on open + `{type:'ping'}` heartbeat; hub→wrapper `{t:'inject',prompt}` + `{type:'pong',data:null}`. Injection bytes (`injection.ts` `bracketedPaste`): `ESC[200~` + sanitized body + `ESC[201~` + `\r` — bracketed paste so the TUI treats it as a literal human paste (newlines stay soft, control bytes aren't keybindings) and the trailing CR submits. `sanitize` strips embedded `ESC[201~` (breakout guard), C0 controls except `\t`/`\n`, DEL, lone CR, and trailing whitespace.
